...
/// - Handles local input.
/// - Spawns initial test items and enemies.
///
/// For NON-PROGRAMMERS:
...
{
    // Legacy UI References (_lobbyPanel, _createRoomButton, _joinRoomButton) have been removed.
    // Room creation and joining UI is now exclusively managed by LobbyUIManager in the Menu scene.
    ...
    private bool worldItemsSpawned = false; // Ensures items/enemies are spawned only once
    ...
    void Start()
    {
        // Legacy button listeners removed. Room joining/creation logic is now triggered automatically 
        // based on RoomConfig data passed from the menu, or generated by the editor auto-host below.

#if UNITY_EDITOR
        // which previously prevented the auto-host block from running in the Editor.
        if ((string.IsNullOrEmpty(RoomConfig.RoomName) || RoomConfig.RoomName == "DefaultRoom") && (_networkRunner != null && _networkRunner.Config == null))
        {
        ...
        {
            // Evaluate the role configured in the menu or by the auto-host
    ...
    private async void CreateRoom()
    {
        ...
        var gameArg = new StartGameArgs()
        {
            ...
            // Inform Fusion of the active scene to awaken Scene Objects (e.g., Damage Zones)
    ...
    public async void JoinSpecificRoom(string sessionName)
    {
        ...
        var gameArg = new StartGameArgs()
        {
            ...
            // Clients also need to know which scene to load for Scene Objects to sync
            Scene = SceneRef.FromIndex(SceneManager.GetActiveScene().buildIndex)
        };
    ...
    public void OnPlayerJoined(NetworkRunner runner, PlayerRef player) 
    {
        ...
        // Reintegrate logic to spawn enemies. (Only happens once, handled by the Host)
        if (!worldItemsSpawned)
        {
            if (_testEnemyPrefab != null)
            {
                // Spawn the enemy a bit further away from the player to prevent immediate collisions
                Vector3 enemySpawnPos = spawnPos + new Vector3(5f, 0, 5f);
                _networkRunner.Spawn(_testEnemyPrefab, enemySpawnPos, Quaternion.identity, null);
                Debug.Log("[NETWORK] Test Enemy Spawned!");
            }
